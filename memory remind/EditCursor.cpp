...
void EditCursor::SetMove(weekListObj objList, const Game_ctr &controller)
{
	...
	{
		if (controller.GetCtr(CONTROLLER_1P_INPUT_BUTTON_X,P_1) == PAD_PUSH)
		{
			switch (count)
			{
			case 1:
	...
	itemButton = controller.GetCtr(CONTROLLER_1P_INPUT_BUTTON_Y,P_1);
	setButton = controller.GetCtr(CONTROLLER_1P_INPUT_BUTTON_B,P_1);
	resetButton = controller.GetCtr(CONTROLLER_1P_INPUT_BUTTON_X,P_1);
